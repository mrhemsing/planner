...
for (const item of targetList) {
  ...
  const instructions = (Array.isArray(data.recipeInstructions) ? data.recipeInstructions : [])
    .flatMap((step) => {
  ...
  detailMap[recipe.id] = {
    ...(parseDuration(data.totalTime || data.prepTime) ? { prepTime: parseDuration(data.totalTime || data.prepTime) } : {}),
...
const outPath = path.join(process.cwd(), "src", "data", "healthy-dinner-details.generated.json");
fs.writeFileSync(outPath, JSON.stringify(detailMap, null, 2) + "\n", "utf8");
